...
public class MqttAuth{
	...
	SSLServerSocket serverSocket = null;
		...
		try{
			...
			serverSocket=(SSLServerSocket)SSL_SOCKET_FACTORY.createServerSocket(10000);
...
class ServerThread extends Thread{
	...
	public ServerThread(Socket sct){
		soc=sct;
		System.out.println("create new connection.  Connect to " + soc.getInetAddress());
	}
	...
	public static void RMQSignUP(String user, String hash_pass){
		...
		try{
			ProcessBuilder pb = new ProcessBuilder("sudo", "rabbitmqctl", "add_user", user, hash_pass);
			Process process1 = pb.start();
			ProcessBuilder pems = new ProcessBuilder("sudo", "rabbitmqctl", "set_permissions", user, ".*", ".*", ".*");
			Process process2 = pems.start();
	...
	public void run(){
		...
		try{
			...
			if(succeeded){
				System.out.println("Authentication: Successfully ->  " + soc.getInetAddress());
				...
				byte[] keyBytes = generatedKey("hello, world");
				//共通鍵送信
				OutputStream os = soc.getOutputStream();
				os.write(keyBytes);
				os.flush();
				//共通鍵確認用
				System.out.println(printBytes(keyBytes));
			


			}else{
				System.out.println("Authentication: Invalid password -> " + soc.getInetAddress());
